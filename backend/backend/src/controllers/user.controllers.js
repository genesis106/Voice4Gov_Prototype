...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // 5️⃣ Fetch user without sensitive fields
    ...
    // 6️⃣ Configure secure cookies
    const cookieOptions = {
    httpOnly: true,
    secure: false, // false for localhost development
    sameSite: "lax",
    domain: "localhost", // ✅ This allows cookies to work across all localhost ports
    path: "/",
    maxAge: 24 * 60 * 60 * 1000,
};

    // 7️⃣ Set cookies and return response
    // ❌ Only refresh token in cookie for security
    // ✅ Access token returned in JSON for frontend usage
    return res.status(200)
    .cookie("accessToken", accessToken, cookieOptions)      // ✅ ADD THIS
...
const logoutUser = asyncHandler( async (req, res) => {
    ...
    sameSite: 'lax',
    domain: 'localhost'
    // No maxAge needed for clearing cookies
}
